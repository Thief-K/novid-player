...
export const SettingsModal: React.FC = () => {
  ...
  const handleCopyDiagnostic = async () => {
    const diagInfo = [
      "### NovidPlayer Diagnostic Info",
      `- **App Version**: v1.0.0`,
      `- **Platform**: Windows 11`,
      `- **User Agent**: ${navigator.userAgent}`,
      `- **Display**: ${window.screen.width}x${window.screen.height} (DPR: ${window.devicePixelRatio})`,
      `- **Hardware Decoding**: Preference: ${hwdec} | Active: ${hwdecCurrent || "N/A"}`,
      `- **Auto-Fit Window**: ${autoFitWindow ? "Enabled" : "Disabled"}`,
      `- **Language**: ${language}`,
      `- **Current Media**: ${currentPath ? "Loaded" : "None"}`,
    ...
    try {
      ...
      setCopiedDiag(true);
      addToast(t("settings.copyDiagnostic"), t("settings.diagnosticCopied"), "success");
    ...
    >
      ...
      >
        ...
        <div className="p-5 overflow-y-auto max-h-[62vh] min-h-[290px] flex flex-col justify-between">
          ...
          {activeTab === "about" && (
            <div className="flex flex-col items-center justify-center text-center space-y-4 py-4 animate-in fade-in duration-200">
              <NovidLogo size={56} glow />
              ...
              <div className="flex items-center justify-center gap-2.5 pt-1">
                <button
                  type="button"
                  onClick={() =>
                    mpvService.openExternalUrl("https://github.com/Thief-K/novid-player")
                  }
                  className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-medium bg-slate-800/80 hover:bg-slate-700/80 text-slate-200 hover:text-white border border-slate-700/60 shadow-xs transition-all active:scale-95 cursor-pointer"
                >
                ...
                >
                  {copiedDiag ? (
                    <>
                      ...
                      <span className="text-emerald-300">{t("settings.diagnosticCopied")}</span>
                    </>
                  ) : (
                    <>
                      <Copy className="w-3.5 h-3.5 text-sky-400" />
                      <span>{t("settings.copyDiagnostic")}</span>
                    </>
                  )}
                </button>
              </div>
              ...
              <div className="text-[10px] text-slate-500 font-mono pt-2">
                GPL-3.0 Licensed · Direct3D 11 & DirectComposition
              </div>
            </div>
          )}
        </div>
